...
BOOL CIRCFrame::PreTranslateMessage(MSG* pMsg) {
    ...
    case WM_NOTIFY:
        switch (pMsg->wParam) {
        case IDC_IRC_DBLCLKCHANNELS: ChanListDblClick(); return TRUE;
        case IDC_IRC_DBLCLKUSERS: UserListDblClick(); return TRUE;
        case IDC_IRC_MENUUSERS: Skin.TrackPopupMenu(L"CIRCUserList", pMsg->pt); return TRUE;
        }
        break;
    case WM_MOUSEMOVE:
        if (m_wndTab.m_nHoverTab > -1) m_wndTab.PreTranslateMessage(pMsg);
        break;
    case WM_KEYDOWN:
        ...
        if (GetFocus() == &m_wndEdit) {
            ...
            case VK_UP:
            case VK_PRIOR:
                if (m_nCurrentPosLineBuffer[nTab] + 1 < m_pLastLineBuffer[nTab].GetCount()) {
            ...
            case VK_DOWN:
            case VK_NEXT:
                if (m_nCurrentPosLineBuffer[nTab] >= 0) {
                    m_nCurrentPosLineBuffer[nTab]--;
                    m_wndEdit.SetWindowText(m_nCurrentPosLineBuffer[nTab] == -1 ? m_sTemp : m_pLastLineBuffer[nTab].GetAt(m_nCurrentPosLineBuffer[nTab]));
                }
                return TRUE;
            case VK_END:
            case VK_HOME:
            ...
            switch (pMsg->wParam) {
            case VK_UP:
            case VK_LEFT: scrollType = SB_LINEUP; break;
            case VK_DOWN:
            case VK_RIGHT: scrollType = SB_LINEDOWN; break;
            case VK_PRIOR: scrollType = SB_PAGEUP; break;
            case VK_NEXT: scrollType = SB_PAGEDOWN; break;
            case VK_HOME: scrollType = SB_TOP; break;
            case VK_END: scrollType = SB_BOTTOM; break;
            default: return CWnd::PreTranslateMessage(pMsg);
            }
            m_wndView.OnVScroll(scrollType, GetScrollPos(SB_VERT));
            return TRUE;
        }
        break;
    default:
        if (pMsg->wParam == IDC_IRC_CHANNELS) {
            if (pMsg->message == WM_ADDCHANNEL) {
                ...
                if (strChan.GetAt(0) != '#') strChan = L"#" + strChan;
                ...
                if (nItem != -1 && m_pChanList.GetType(m_wndPanel.m_boxChans.m_wndChanList.GetItemText(nItem, 0))) {

...
void CSymEngine::GetOsInfo(COsInfo &rOsInfo) {
    static const TCHAR* OS_VERSIONS[] = {
        _T("Unknown"),
        _T("Windows NT 4.0"), _T("Windows 95"), _T("Windows 98"), _T("Windows Me"),
        _T("Windows 2000"), _T("Windows XP"), _T("Windows XP x64"),
        _T("Windows Vista"), _T("Windows 7"), _T("Windows 8"), _T("Windows 8.1"),
        _T("Windows 10"), _T("Windows 11"),
        _T("Windows Server 2003"), _T("Windows Home Server"), _T("Windows Server 2003 R2"),
        _T("Windows Server 2008"), _T("Windows Server 2008 R2"),
        _T("Windows Server 2012"), _T("Windows Server 2012 R2"),
        _T("Windows Server 2016"), _T("Windows Server 2019"), _T("Windows Server 2022")
    };
    ...
    rOsInfo.m_pszWinVersion = OS_VERSIONS[0];  // Unknown by default

    if (osvi.dwPlatformId == VER_PLATFORM_WIN32_NT) {
        switch (osvi.dwMajorVersion) {
        case 4:
            rOsInfo.m_pszWinVersion = OS_VERSIONS[1];  // Windows NT 4.0
            break;
        case 5:
            switch (osvi.dwMinorVersion) {
            case 0: rOsInfo.m_pszWinVersion = OS_VERSIONS[5]; break;  // Windows 2000
            case 1: rOsInfo.m_pszWinVersion = OS_VERSIONS[6]; break;  // Windows XP
            case 2:
                if (osvi.wProductType == VER_NT_WORKSTATION && sysi.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64)
                    rOsInfo.m_pszWinVersion = OS_VERSIONS[7];  // Windows XP x64
                else if (osvi.wSuiteMask & VER_SUITE_WH_SERVER)
                    rOsInfo.m_pszWinVersion = OS_VERSIONS[15];  // Windows Home Server
                else
                    rOsInfo.m_pszWinVersion = GetSystemMetrics(SM_SERVERR2) == 0 ? OS_VERSIONS[14] : OS_VERSIONS[16];  // Server 2003 or 2003 R2
                break;
            }
            break;
        case 6:
            switch (osvi.dwMinorVersion) {
            case 0: rOsInfo.m_pszWinVersion = osvi.wProductType == VER_NT_WORKSTATION ? OS_VERSIONS[8] : OS_VERSIONS[17]; break;  // Vista or Server 2008
            case 1: rOsInfo.m_pszWinVersion = osvi.wProductType == VER_NT_WORKSTATION ? OS_VERSIONS[9] : OS_VERSIONS[18]; break;  // 7 or Server 2008 R2
            case 2: rOsInfo.m_pszWinVersion = osvi.wProductType == VER_NT_WORKSTATION ? OS_VERSIONS[10] : OS_VERSIONS[19]; break;  // 8 or Server 2012
            case 3: rOsInfo.m_pszWinVersion = osvi.wProductType == VER_NT_WORKSTATION ? OS_VERSIONS[11] : OS_VERSIONS[20]; break;  // 8.1 or Server 2012 R2
            }
            break;
        case 10:
            if (osvi.wProductType == VER_NT_WORKSTATION) {
                rOsInfo.m_pszWinVersion = osvi.dwBuildNumber >= 22000 ? OS_VERSIONS[13] : OS_VERSIONS[12];  // 11 or 10
            } else {
                if (osvi.dwBuildNumber >= 20348)
                    rOsInfo.m_pszWinVersion = OS_VERSIONS[23];  // Server 2022
                else if (osvi.dwBuildNumber >= 17763)
                    rOsInfo.m_pszWinVersion = OS_VERSIONS[22];  // Server 2019
                else
                    rOsInfo.m_pszWinVersion = OS_VERSIONS[21];  // Server 2016
            }
            break;
        }
    } else if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
        switch (osvi.dwMinorVersion) {
        case 0: rOsInfo.m_pszWinVersion = OS_VERSIONS[2]; break;  // 95
        case 10: rOsInfo.m_pszWinVersion = OS_VERSIONS[3]; break;  // 98
        case 90: rOsInfo.m_pszWinVersion = OS_VERSIONS[4]; break;  // Me
        }
    }
